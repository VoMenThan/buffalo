// Customized to include the build_nav_callback options
// https://github.com/eliotk/cs_jquery_paginate
  ...
  $.fn.paginate = function(opts) {
    ...
    hide_all = function() {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        x = _ref[_i];
        $(x).removeClass('first-child last-child')
    ...
    show_paged_results = function(active_page) {
      ...
      for (_i = 0, _len = this_page_items.length; _i < _len; _i++) {
        ...
        if( _i == 0 ) $(x).addClass('first-child')
        if( _i == _len - 1 ) $(x).addClass('last-child')
        $(x).show();
      }
    ...
    build_nav = function(active_page) {
      ...
      while (page_count < pages + 1) {
        if (parseInt(active_page) === page_count) {
          page_html += '<li class="paginate-active-page" data-page="' + page_count + '">' + page_count + '</li>';
        } else {
          page_html += '<li data-page="' + page_count + '"><a href="#paginate-anchor" class="paginate-click">' + page_count + '</a></li>';
        }
        page_count++;
      }
      ...
      opts.build_nav_callback()

      $('.paginate-click').click(function(e) {
        var index = $(this).closest('li').data('page')
        if( !opts.scroll_to_top ) e.preventDefault()
        build_nav( index );
        return show_paged_results( index );
      });

      $('.next-click').click(function(e) {
        var index = $('.paginate-active-page').data('page') + 1
        if( !opts.scroll_to_top ) e.preventDefault()
        return show_paged_results( index );
      });

      return $('.prev-click').click(function(e) {
        var index = $('.paginate-active-page').data('page') - 1
        if( !opts.scroll_to_top ) e.preventDefault()
        return show_paged_results( index );
      });
