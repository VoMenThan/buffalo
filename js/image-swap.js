...
!function( $, app ) {
  ...
  defaults = {
    ...
    keepImage: false // if an image data doesn't exist remove the old image
  }
  ...
  ImageSwap = function( options ) {
    ...
    this.$keepImage = options.keepImage
  ...
  ImageSwap.prototype = {
    ...
    setup0_768: function() {
      ...
      this.$mobileImageContainer.each(function(i){
        ...
        if (!mobileImageSrc || !mobileImageSrc.length)  {
          // if we want to keep the image
          if (self.$keepImage) {
            // lets check for an image and if they want to make a new image lets do so
    ...
    setup769up: function() {
      ...
      this.$mobileImageContainer.each(function(i){
        ...
        // if the element doesnt have an attribute we need to kill the empty image
        if (!desktopImageSrc || !desktopImageSrc.length)  {
          if (self.$keepImage) {
            // lets check for an image and if they want to make a new image lets do so
            if (imageTarget.length == 0 && self.$createEmptyImage) {
              el.append('<img src='+ mobileImageSrc +' class='+ self.$mobileImageClass.replace('.', '') +' >')
        ...
        // if it has an attribute to do swapping we can swap, its just incase we dont want to swap
        if( $.hasData(this) ) {
          if(!$(this).data('desktop-img')) return // a bit hacky
          // replace the image source with the new one :)
          if (imageTarget.length == 0 && self.$createEmptyImage) {
            imageTarget = el.append('<img src='+ desktopImageSrc +' class='+ self.$mobileImageClass.replace('.', '') +' >')
          } else {
